...
public class LoggerMessageProcessorTestCase extends AbstractMuleTestCase 
{
    ...
    private void verifyLogCall(KLog loggerMessageProcessor, String logLevel, String enabledLevel, MuleEvent muleEvent, Object objectToLog)
    {
    ...
    private void verifyExpressionEvaluation(KLog loggerMessageProcessor, String level, String enabledLevel, MuleEvent muleEvent, VerificationMode timesEvaluateExpression)
    {
        when(loggerMessageProcessor.logger.isTraceEnabled()).thenReturn("TRACE".equals(enabledLevel));
        when(loggerMessageProcessor.logger.isDebugEnabled()).thenReturn("DEBUG".equals(enabledLevel));
        when(loggerMessageProcessor.logger.isInfoEnabled()).thenReturn("INFO".equals(enabledLevel));
        ...
        loggerMessageProcessor.log(muleEvent);
        verify(loggerMessageProcessor.expressionManager,(VerificationMode)timesEvaluateExpression).parse("some expression", muleEvent);
    }
    
    // Orchestrates the verifications for a call with a null MuleEvent
    private void verifyNullEventByLevel(String level)
    {
        KLog loggerMessageProcessor = buildLoggerMessageProcessorWithLevel(level);
    ...
    private void verifyLoggerMessageByLevel(String level)
    {
        ...
        verifyExpressionEvaluation(buildLoggerMessageProcessorForExpressionEvaluation(level), level, level, muleEvent, (VerificationMode) times(1)); // Expression should be evaluated when the level is enabled
      
        verifyExpressionEvaluation(buildLoggerMessageProcessorForExpressionEvaluation(level), level, "not"+ level, muleEvent, (VerificationMode) never()); // Expression should not be evaluated when the level is enabled
    }
    ...
    private KLog buildLoggerMessageProcessorWithLevel(String level)
    {
        KLog loggerMessageProcessor = new KLog();
    ...
    private KLog buildLoggerMessageProcessorForExpressionEvaluation(String level)
    {
        KLog loggerMessageProcessor = buildLoggerMessageProcessorWithLevel(level);
        loggerMessageProcessor = buildLoggerMessageProcessorWithLevel(level);
        loggerMessageProcessor.expressionManager = buildExpressionManager();
